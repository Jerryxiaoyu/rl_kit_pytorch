"""This file implements the functionalities of a laikago using pybullet.
...
pybullet_model_path = os.getenv('PYBULLET_MODELS_PATH')
print('Pybullet_model_path is imported: ', pybullet_model_path)

INIT_POSITION = [0, 0, .45] #checked
INIT_RACK_POSITION = [0, 0, 1] #checked
INIT_ORIENTATION = [0, 0, 0, 1]
KNEE_CONSTRAINT_POINT_RIGHT = [0, 0.005, 0.2]
KNEE_CONSTRAINT_POINT_LEFT = [0, 0.01, 0.2]
OVERHEAT_SHUTDOWN_TORQUE = 2.45
OVERHEAT_SHUTDOWN_TIME = 1.0
LEG_POSITION = ["FL", "RL", "FR", "RR"]  # foward left, Hind_left, foward right, hind right
MOTOR_NAMES = [
...
INIT_JOINT_ANGLE = np.array([0, 0.62, -1.26, 0, 0.62, -1.26, 0, 0.62, -1.26, 0, 0.62, -1.26, ])
...
LOWER_ANGLE_JOINT = [-0.872664625997, -0.523598775598, -2.77507351067 ,
                     ...
                     -0.872664625997, -0.523598775598, -2.77507351067,
                     -0.872664625997, -0.523598775598, -2.77507351067]
MAX_FORCE_JOINT =    [52.4, 55, 55,
                      52.4, 55, 55,
                      52.4, 55, 55,
                      52.4, 55, 55, ]
MAX_VELOCITY_JOINT = [20,28.6, 28.6,
                      20, 28.6, 28.6,
                      20, 28.6, 28.6,
                      20, 28.6, 28.6]
MIN_FORCE_JOINT = (-np.array(MAX_FORCE_JOINT)).tolist()
MIN_VELOCITY_JOINT = (-np.array(MAX_VELOCITY_JOINT)).tolist()
...
class Laikago(object):
  """The laikago class that simulates a quadruped robot .

  """

  def __init__(self,
               pybullet_client,
               urdf_root=pybullet_model_path,
               time_step=0.01,
               action_repeat=1,
               ...
               on_rack=False):
    """Constructs a laikago and reset it to the initial states.
    ...
    self._motor_direction = np.ones(self.num_motors)  #TODO
    self._observed_motor_torques = np.zeros(self.num_motors)
    self._applied_motor_torques = np.zeros(self.num_motors)
    self._max_force = 50  #TODO
    ...
    self.lower_angle = np.array(LOWER_ANGLE_JOINT)
    self.upper_angle = np.array(UPPER_ANGLE_JOINT)
    self.lower_torque = np.array(MAX_FORCE_JOINT) * (-1)
    self.upper_torque = np.array(MAX_FORCE_JOINT)
    ...
    if self._torque_control_enabled:
        print('The control mode of Laikago is TORQUE MODE.')
    else:
        print('The control mode of Laikago is POSITION MODE.')
    # reset_time=-1.0 means skipping the reset motion.
    # See Reset for more details.
    self.Reset(reset_time=-1.0)

  def GetTimeSinceReset(self): #checked
  ...
  def Reset(self, reload_urdf=True, default_motor_angles=None, reset_time=3.0):
    ...
    if reload_urdf:
      if self._self_collision_enabled:
        self.quadruped = self._pybullet_client.loadURDF(
            "%s/laikago_v1/laikago2.1.urdf" % self._urdf_root,
            init_position,
    ...
    self._init_angle = INIT_JOINT_ANGLE
    ...
    if not self._torque_control_enabled and reset_time > 0.0:
      self.ReceiveObservation()
      for _ in range(100):
        self.ApplyAction(self._init_angle )
        self._pybullet_client.stepSimulation()
        self.ReceiveObservation()
  ...
  def _ResetPoseForLeg(self, leg_id, add_constraint):
    ...
    self._pybullet_client.resetJointState(
        self.quadruped,
        self._joint_name_to_id[leg_position + '_hip' +"_joint"],
        hip,
        targetVelocity=0)
    self._pybullet_client.resetJointState(
        self.quadruped,
        self._joint_name_to_id[leg_position + '_thigh' +"_joint"],
         thigh,
        targetVelocity=0)
    self._pybullet_client.resetJointState(
        self.quadruped,
        self._joint_name_to_id[leg_position + '_calf' +"_joint"],
         calf,
        targetVelocity=0)
    ...
    if self._accurate_motor_model_enabled or self._pd_control_enabled:
      ...
      self._SetDesiredMotorAngleByName(
          leg_position + '_hip' +"_joint",
          hip)
      self._SetDesiredMotorAngleByName(
          leg_position + '_thigh' +"_joint",
          thigh)
      self._SetDesiredMotorAngleByName(
          leg_position + '_calf' + "_joint",
          calf)
  ...
  def GetJointReactionForces(self):
      reaction_forces = [
          self._pybullet_client.getJointState(self.quadruped, motor_id)[2]
          for motor_id in self._motor_id_list ]
      return np.array(reaction_forces)
  ...
  def GetObservationUpperBound(self):
    ...
    upper_bound[0:self.num_motors] = UPPER_ANGLE_JOINT  # Joint angle.
    upper_bound[self.num_motors:2 * self.num_motors] = MAX_VELOCITY_JOINT  #    Joint velocity.
    upper_bound[2 * self.num_motors:3 * self.num_motors] = MAX_FORCE_JOINT # Joint torque.
  ...
  def GetObservationLowerBound(self):
    ...
    lower_bound[0:self.num_motors] = LOWER_ANGLE_JOINT  # Joint angle.
    lower_bound[self.num_motors:2 * self.num_motors] = MIN_VELOCITY_JOINT  # Joint velocity.
    lower_bound[2 * self.num_motors:3 * self.num_motors] = MIN_VELOCITY_JOINT  # Joint torque.
    lower_bound[3 * self.num_motors:] = 1.0
    return lower_bound
  ...
  def ApplyAction(self, motor_commands, motor_kps=None, motor_kds=None):
    """Set the desired motor angles to the motors of the Laikago.
    ...
    Args:
      ...
      motor_kps: Proportional gains for the motor model. If not provided, it
        uses the default kp of the Laikago for all the motors.
      motor_kds: Derivative gains for the motor model. If not provided, it
        uses the default kd of the Laikago for all the motors.
    """
    ...
    if not self._torque_control_enabled:
        ...
        if self._accurate_motor_model_enabled or self._pd_control_enabled:
          ...
          for motor_id, motor_command in zip(self._motor_id_list, motor_commands):
            self._SetMotorTorqueById(motor_id, motor_command)
  ...
  def ConvertFromLegModel(self, actions):
    ...
    motor_angle = actions
    # lower_angle = np.array(LOWER_ANGLE_JOINT)
    # upper_angle = np.array(UPPER_ANGLE_JOINT)
    #
    #
    # motor_angle =
    return motor_angle
  def ConvertAction2Angle(self, actions, action_bound=1):
    """Convert the actions that use leg model to the real motor actions.
    ...
    motor_angle = self.lower_angle + (self.upper_angle-self.lower_angle)/(2*action_bound)*(actions+action_bound)

    return motor_angle



  def ConvertAction2Torque(self, actions, action_bound=1):
      """Convert the actions that use leg model to the real motor actions.
      ...
      Returns:
        The eight desired motor angles that can be used in ApplyActions().
      """
      motor_torque = self.lower_torque + (self.upper_torque - self.lower_torque) / (2 * action_bound) * (
              actions + action_bound)

      return motor_torque
  def ConvertMotorangle2Action(self, motor_angle, action_bound=1):
      """Convert the actions that use leg model to the real motor actions.
      ...
      actions  = 2*action_bound *(motor_angle -self.lower_angle)/(self.upper_angle - self.lower_angle)  -action_bound
  ...
  def SetBaseMasses(self, base_mass):
    """Set the mass of Laikago's base.
  ...
  def SetBaseInertias(self, base_inertias):
    """Set the inertias of Laikago's base.
